...
impl Ufile for LocalFile {
    ...
    fn update_disk_image(&mut self, file: File) -> io::Result<()> {
        info!("update virtio-blk image file.");
        self.file = ManuallyDrop::new(file);
        self.capacity = self.file.seek(SeekFrom::End(0))?;
        Ok(())
    }
    ...
    fn restore_io_context(&mut self) -> io::Result<()> {
        self.aio_context = match IoContext::new(256) {
            Ok(c) => c,
            Err(e) => {
                error!("restore_io_context: create new aio context: {}", e);
                return Err(e);
            }
        };
        Ok(())
    ...
    // Though AIO context won't be available in the child process, we
    // don't have to destroy it explicitly (different with the TDC case,
    // which the backend service would check if there exists more than
    // one connection). And Block::liveupgrade_restore would create a new
    // context for both localfile and TDC case without checking any flags.
    // In other words, this function is only used by liveupgrade_rollback
    // for now (in the parent process).

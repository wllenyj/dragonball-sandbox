...
pub struct MmioV2DeviceState<AS: GuestAddressSpace + Clone> {
...
impl<AS: GuestAddressSpace + Clone> MmioV2DeviceState<AS> {
    /// Returns a reference to the internal device object.
    pub fn get_inner_device(&self) -> &dyn VirtioDevice<AS> {
        self.device.as_ref()
    }

    /// Returns a mutable reference to the internal device object.
    pub fn get_inner_device_mut(&mut self) -> &mut dyn VirtioDevice<AS> {
        self.device.as_mut()
    }

    pub(crate) fn new(
        mut device: Box<dyn VirtioDevice<AS>>,
        vm_fd: Arc<VmFd>,
        vm_as: AS,
    ...
    fn create_queues(device: &dyn VirtioDevice<AS>) -> Result<(Vec<VirtioQueueConfig>, bool)> {
        ...
        if has_ctrl_queue {
            let notifier = Box::new(NoopNotifier::new());
            queues.push(VirtioQueueConfig::new(
                QueueState::new(device.ctrl_queue_max_sizes()),
                EventFd::new(0)?,
                notifier,
                queues.len() as u16,
            ));
...
impl<AS: GuestAddressSpace + Clone> Drop for MmioV2DeviceState<AS> {
    fn drop(&mut self) {

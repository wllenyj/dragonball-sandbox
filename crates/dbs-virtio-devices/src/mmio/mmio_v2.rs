...
pub struct MmioV2Device<AS: GuestAddressSpace + Clone> {
    state: Mutex<MmioV2DeviceState<AS>>,
...
impl<AS: GuestAddressSpace + Clone> MmioV2Device<AS> {
    /// Constructs a new MMIO transport for the given virtio device.
    pub fn new(
        vm_fd: Arc<VmFd>,
        vm_as: AS,
        irq_manager: Arc<KvmIrqManager>,
        device: Box<dyn VirtioDevice<AS>>,
    ...
    pub fn state(&self) -> MutexGuard<MmioV2DeviceState<AS>> {
    ...
    fn update_queue_field<F: FnOnce(&mut QueueState)>(&self, f: F) {
...
impl<AS: 'static + GuestAddressSpace + Clone + Send + Sync> DeviceIo for MmioV2Device<AS> {

...
impl<AS: GuestAddressSpace> PartialEq for Net<AS> {
    fn eq(&self, other: &Net<AS>) -> bool {
        self.device_info == other.device_info
            && self.queue_sizes == other.queue_sizes
            && self.rx_rate_limiter.is_some() == other.rx_rate_limiter.is_some()
            && self.tx_rate_limiter.is_some() == other.tx_rate_limiter.is_some()
    }
}
